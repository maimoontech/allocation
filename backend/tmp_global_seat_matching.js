...
(async () => {
  ...
  const partyIndexById = new Map(partyPrefs.map((p, i) => [p.id, i]));

  function tryMatch(partyIndex, seen) {
    for (const seatIndex of partyPrefs[partyIndex].seats) {
      ...
      if (currentPartyIndex === -1 || tryMatch(currentPartyIndex, seen)) {
  ...
  for (let partyIndex = 0; partyIndex < partyPrefs.length; partyIndex += 1) {
    if (tryMatch(partyIndex, new Set())) matched += 1;
  }
