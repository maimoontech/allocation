...
export async function generateSchedule(params: {
  ...
  try {
    ...
    const seatList: Array<{ venueId: number; round: number; stage: 1 | 2 | 3 }> = [];
    const maxRounds = Math.max(...venues.map((venue) => venue.max_parties));
    for (let round = 1; round <= maxRounds; round += 1) {
      for (const venue of venues) {
        if (round <= venue.max_parties) {
          const stage: 1 | 2 | 3 = round === 1 ? 1 : round <= venue.min_parties ? 2 : 3;
          seatList.push({ venueId: venue.id, round, stage });
        }
      }
    }

    function buildMatchableParties(
      candidateParties: Party[],
      canUseSeat: (partyId: number, venueId: number) => boolean,
          ...
          const seatIndexes = seatList
            ...
            .sort((a, b) => {
              if (preferUnvisited) {
                ...
                if (revisitPenaltyA !== revisitPenaltyB) return revisitPenaltyA - revisitPenaltyB;
              }
    ...
    function maximizeAssignments(
      matchableParties: Array<Party & { seatIndexes: number[] }>,
      allowedSeatIndexes: Set<number>
    ) {
    ...
    const strictlyAssignedPartyIds = new Set<number>();
    ...
    const remainingSeatIndexes = new Set(
      strictSeatToPartyIndex.map((partyIndex, index) => (partyIndex < 0 ? index : -1)).filter((index) => index >= 0)
    );
    const relaxedMatchableParties = buildMatchableParties(relaxedCandidateParties, () => true, true);
    const relaxedSeatToPartyIndex =
      relaxedMatchableParties.length > 0 && remainingSeatIndexes.size > 0
        ? maximizeAssignments(relaxedMatchableParties, remainingSeatIndexes)
        : Array<number>(seatList.length).fill(-1);
