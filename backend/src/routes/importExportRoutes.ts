...
function toCsv(rows: Array<Record<string, unknown>>, headers: string[]) {
  ...
  for (const r of rows) {
    lines.push(headers.map((h) => escapeCsvField((r as any)[h])).join(","));
  }
  return lines.join("\r\n");
}

function asBool01(v: string | undefined) {
...
const entityConfig = {
  ...
  parties: {
    templateHeaders: ["zone_name", "its_no", "leader_name", "contact_number", "whatsapp_number", "party_name", "category", "is_active", "password"],
    exportHeaders: ["zone_name", "its_no", "leader_name", "contact_number", "whatsapp_number", "party_name", "category", "is_active", "password"]
  },
  venues: {
    templateHeaders: [
      "zone_name",
...
importExportRoutes.get("/:entity/template", async (req, res) => {
  ...
  const cfg = (entityConfig as any)[entity];
  if (!cfg) return fail(res, "Invalid entity", 400);

  const csv = toCsv([], cfg.templateHeaders);
  return csvResponse(res, `${entity}_template.csv`, csv);
});

importExportRoutes.get("/:entity/export", async (req, res) => {
  const entity = String(req.params.entity ?? "");
  const cfg = (entityConfig as any)[entity];
  if (!cfg) return fail(res, "Invalid entity", 400);
  ...
  if (entity === "zones") {
    const [rows] = await pool.query<any[]>(
      "SELECT zone_name, coordinator_name, contact_number, whatsapp_number, '' AS password FROM zones ORDER BY zone_name"
    );
  ...
  if (entity === "mohallahs") {
    const [rows] = await pool.query<any[]>(
      `SELECT z.zone_name, m.mohallah_name, m.coordinator_name, m.contact_number, m.whatsapp_number
  ...
  if (entity === "parties") {
    const [rows] = await pool.query<any[]>(
      `SELECT z.zone_name, p.its_no, p.leader_name, p.contact_number, p.whatsapp_number, p.party_name, p.category, p.is_active, '' AS password
  ...
  if (entity === "venues") {
    const [rows] = await pool.query<any[]>(
      `SELECT z.zone_name, m.mohallah_name, v.venue_name, v.coordinator_name, v.contact_number, v.whatsapp_number,
  ...
  if (entity === "miqaats") {
    const [rows] = await pool.query<any[]>(
      `SELECT miqaat_name, DATE_FORMAT(english_date, '%Y-%m-%d') AS english_date, hijri_date, is_active
       FROM miqaats ORDER BY english_date DESC`
    );
    return csvResponse(res, "miqaats.csv", toCsv(rows, cfg.exportHeaders));
  }

  return fail(res, "Invalid entity", 400);
});

importExportRoutes.post("/:entity/import", async (req, res) => {
  const entity = String(req.params.entity ?? "");
  const cfg = (entityConfig as any)[entity];
  if (!cfg) return fail(res, "Invalid entity", 400);

  const csvText = String(req.body?.csv ?? "");
  if (!csvText.trim()) return fail(res, "csv is required", 400);

  const table = parseCsv(csvText);
  if (table.length < 2) return fail(res, "CSV must include header and at least one row", 400);
  ...
  const requiredHeaders = cfg.templateHeaders.map((h: string) => normalizeHeader(h));
  for (const h of requiredHeaders) {
    if (!(h in headerIndex)) return fail(res, `Missing column: ${h}`, 400);
  }
  ...
  try {
    ...
    const [zones] = await conn.query<any[]>("SELECT id, zone_name FROM zones");
    const zoneByName = new Map<string, number>();
    for (const z of zones) zoneByName.set(String(z.zone_name).trim(), Number(z.id));

    const [mohallahs] = await conn.query<any[]>(
      `SELECT m.id, m.mohallah_name, z.zone_name
       FROM mohallahs m JOIN zones z ON z.id = m.zone_id`
    );
    const mohallahByKey = new Map<string, number>();
    for (const m of mohallahs) {
      const key = `${String(m.zone_name).trim()}||${String(m.mohallah_name).trim()}`;
      mohallahByKey.set(key, Number(m.id));
    }

    const [parties] = await conn.query<any[]>(
      `SELECT p.id, p.party_name, z.zone_name
       FROM parties p JOIN zones z ON z.id = p.zone_id`
    );
    const partyByKey = new Map<string, number>();
    for (const p of parties) {
      const key = `${String(p.zone_name).trim()}||${String(p.party_name).trim()}`;
      partyByKey.set(key, Number(p.id));
    }

    const [miqaats] = await conn.query<any[]>(
      "SELECT id, miqaat_name, DATE_FORMAT(english_date, '%Y-%m-%d') AS english_date FROM miqaats"
    );
    const miqaatByKey = new Map<string, number>();
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      const rowNumber = i + 2;
      const get = (k: string) => String(row[headerIndex[normalizeHeader(k)]] ?? "").trim();
      ...
      try {
        if (entity === "zones") {
          ...
          if (!zone_name || !coordinator_name) throw new Error("zone_name and coordinator_name are required");
          ...
          if (existingId) {
            ...
            if (affected === 1) result.updated += 1;
            else result.skipped += 1;
          } else {
            if (!password) throw new Error("password is required for new zone");
            const password_hash = await bcrypt.hash(String(password), 10);
            const [r] = await conn.query<any>(
              `INSERT INTO zones (zone_name, coordinator_name, contact_number, whatsapp_number, password_hash, created_at, updated_at)
            ...
            if (Number.isFinite(insertId) && insertId > 0) zoneByName.set(zone_name, insertId);
          }
          continue;
        }
        ...
        if (entity === "mohallahs") {
          ...
          if (existingId) {
            ...
            const [r] = await conn.query<any>(
              `INSERT INTO mohallahs (zone_id, mohallah_name, coordinator_name, contact_number, whatsapp_number, created_at)
            ...
            result.inserted += 1;
            const insertId = Number((r as any).insertId ?? 0);
            if (Number.isFinite(insertId) && insertId > 0) mohallahByKey.set(key, insertId);
          }
          continue;
        }
        ...
        if (entity === "parties") {
          ...
          if (existingId) {
            ...
            const password_hash = await bcrypt.hash(String(password), 10);
            const [r] = await conn.query<any>(
              `INSERT INTO parties (its_no, leader_name, contact_number, whatsapp_number, party_name, zone_id, category, is_active, password_hash, created_at)
               VALUES (:its_no, :leader_name, :contact_number, :whatsapp_number, :party_name, :zone_id, :category, :is_active, :password_hash, NOW())`,
              { its_no, leader_name, contact_number, whatsapp_number, party_name, zone_id, category, is_active: is_active ?? 1, password_hash }
            );
            result.inserted += 1;
            const insertId = Number((r as any).insertId ?? 0);
            if (Number.isFinite(insertId) && insertId > 0) partyByKey.set(key, insertId);
          }
          continue;
        }
        ...
        if (entity === "venues") {
          ...
          const is_active = asBool01(get("is_active"));
          if (!zone_name || !mohallah_name || !venue_name) throw new Error("zone_name, mohallah_name and venue_name are required");
          const mohallah_id = mohallahByKey.get(`${zone_name}||${mohallah_name}`);
          if (!mohallah_id) throw new Error(`Mohallah not found: ${zone_name} / ${mohallah_name}`);
          ...
          const [r] = await conn.query<any>(
            `INSERT INTO venues
             ...
             ON DUPLICATE KEY UPDATE
               ...
               is_active = VALUES(is_active)${passwordSql}`,
            params
          );
          const affected = Number((r as any).affectedRows ?? 0);
          if (affected === 1) result.inserted += 1;
          else if (affected === 2) result.updated += 1;
          else result.skipped += 1;
          continue;
        }
        ...
        if (entity === "miqaats") {
          ...
          const is_active = asBool01(get("is_active"));
          if (!miqaat_name || !english_date) throw new Error("miqaat_name and english_date are required");
          if (!/^\d{4}-\d{2}-\d{2}$/.test(english_date)) throw new Error("english_date must be YYYY-MM-DD");
          ...
          if (existingId) {
            const [r] = await conn.query<any>(
              `UPDATE miqaats
               ...
               WHERE id = :id`,
              { id: existingId, hijri_date, is_active: is_active ?? 1 }
            );
            const affected = Number((r as any).affectedRows ?? 0);
            if (affected === 1) result.updated += 1;
            else result.skipped += 1;
            ...
            const [r] = await conn.query<any>(
              `INSERT INTO miqaats (miqaat_name, english_date, hijri_date, is_active)
               VALUES (:miqaat_name, :english_date, :hijri_date, :is_active)`,
              { miqaat_name, english_date, hijri_date, is_active: is_active ?? 1 }
            );
            const affected = Number((r as any).affectedRows ?? 0);
            if (affected === 1) result.inserted += 1;
            else result.skipped += 1;
    ...
    await conn.rollback();
    return fail(res, "Import failed", 500);
  } finally {
    conn.release();
  }
