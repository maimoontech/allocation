...
(async()=>{
  ...
  function canAssignPartyToVenue(partyId, venueId){ const covered=currentCycleCoveredByParty.get(partyId); return !covered?.has(venueId); }
  function visitCount(partyId, venueId){ return pairVisitCounts.get(`${partyId}:${venueId}`) || 0; }
  const matchableParties = parties.map((party)=>{
    const seatIndexes = seatList.map((seat,index)=>({seat,index})).filter(({seat})=>canAssignPartyToVenue(party.id, seat.venueId)).sort((a,b)=>{
  ...
  const tryMatch = (partyIndex, seenSeats, allowedSeatIndexes)=>{
    for(const seatIndex of matchableParties[partyIndex].seatIndexes){
      ...
      if(currentPartyIndex === -1 || tryMatch(currentPartyIndex, seenSeats, allowedSeatIndexes)) { seatToPartyIndex[seatIndex]=partyIndex; return true; }
    }
    return false;
  };
  const matchSubset=(partyIndexes, allowedSeatIndexes)=>{ for(const partyIndex of partyIndexes){ if(seatToPartyIndex.includes(partyIndex)) continue; tryMatch(partyIndex,new Set(),allowedSeatIndexes);} };
  const roundOneSeats = new Set(seatList.map((seat,index)=>({seat,index})).filter(({seat})=>seat.round===1).map(({index})=>index));
  const minimumFillSeats = new Set(seatList.map((seat,index)=>({seat,index})).filter(({seat})=>{ const venue=venues.find(v=>v.id===seat.venueId); return seat.round>1 && seat.round<=venue.min_parties;}).map(({index})=>index));
  const categoryIndexes = {A:[],B:[],C:[]};
  matchableParties.forEach((party,index)=>{ if(categoryIndexes[party.category]) categoryIndexes[party.category].push(index); });
  ...
  const remainingSeats = new Set(seatList.map((_,index)=>index).filter(index=>seatToPartyIndex[index] < 0));
  for(let partyIndex=0; partyIndex<matchableParties.length; partyIndex++) { if(seatToPartyIndex.includes(partyIndex)) continue; tryMatch(partyIndex, new Set(), remainingSeats); }
  const assigned = new Set(seatToPartyIndex.filter(i=>i>=0));
  const unassigned = matchableParties.filter((_,index)=>!assigned.has(index));
  const venueById = new Map(venues.map(v=>[v.id,v.venue_name]));
  console.log('Preview miqaat', miqaatId, miqaatRow.miqaat_name, 'assignments=', assigned.size);
  console.log('Unassigned parties:');
  for (const p of unassigned) {
    const allowed = p.seatIndexes.map(i=>seatList[i]).filter((seat, idx, arr)=>arr.findIndex(s=>s.venueId===seat.venueId)===idx).map(seat=>venueById.get(seat.venueId));
    console.log(`- ${p.party_name} (${p.category}) allowed=${JSON.stringify(allowed)}`);
  }
  const occupancy = new Map();
  seatToPartyIndex.forEach((partyIndex, seatIndex)=>{ if(partyIndex<0) return; const seat = seatList[seatIndex]; occupancy.set(seat.venueId, (occupancy.get(seat.venueId)||0)+1);});
  console.log('Occupancy:', JSON.stringify(venues.map(v=>({venue:v.venue_name, min:v.min_parties, max:v.max_parties, used:occupancy.get(v.id)||0}))));
  await conn.end();
