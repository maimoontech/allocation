...
exports.reportsRoutes.use(auth_1.requireAuth, (0, auth_1.requireRole)(["admin", "zonal_head"]));
function getZoneScope(user, rawZoneId) {
    if (user.role === "zonal_head")
        return user.zoneId;
    const zoneId = rawZoneId ? Number(rawZoneId) : null;
    return Number.isFinite(zoneId) ? zoneId : null;
}
function completedPartyMiqaatJoin(scheduleAlias) {
    return `JOIN (
...
exports.reportsRoutes.get("/status-summary", async (req, res) => {
    const user = req.user;
    ...
    const [[partyCounts]] = await pool_1.pool.query(`SELECT
     ...
     FROM parties
     ${zoneWhere}`, params);
    const [[venueCounts]] = await pool_1.pool.query(`SELECT
     ...
     ${Number.isFinite(zoneId) ? "WHERE m.zone_id = :zone_id" : ""}`, params);
...
exports.reportsRoutes.get("/zone-schedule", async (req, res) => {
    const user = req.user;
    const miqaatId = req.query.miqaat_id ? Number(req.query.miqaat_id) : null;
    if (!Number.isFinite(miqaatId))
        return (0, response_1.fail)(res, "miqaat_id is required", 400);
    const zoneId = getZoneScope(user, req.query.zone_id);
    const params = { miqaat_id: miqaatId };
    let zoneSql = "";
    if (Number.isFinite(zoneId)) {
        zoneSql = " AND m.zone_id = :zone_id";
        params.zone_id = zoneId;
    }
    const [rows] = await pool_1.pool.query(`SELECT z.zone_name, m.mohallah_name, v.venue_name,
...
exports.reportsRoutes.get("/party-history", async (req, res) => {
    const user = req.user;
    const partyId = req.query.party_id ? Number(req.query.party_id) : null;
    if (!Number.isFinite(partyId))
        return (0, response_1.fail)(res, "party_id is required", 400);
    if (user.role === "zonal_head") {
        const [rows] = await pool_1.pool.query("SELECT zone_id FROM parties WHERE id = :id LIMIT 1", { id: partyId });
...
exports.reportsRoutes.get("/miqaat-schedule", async (req, res) => {
    const user = req.user;
    const miqaatId = req.query.miqaat_id ? Number(req.query.miqaat_id) : null;
    if (!Number.isFinite(miqaatId))
        return (0, response_1.fail)(res, "miqaat_id is required", 400);
    ...
    const [rows] = await pool_1.pool.query(`SELECT s.id, z.zone_name, m.mohallah_name, v.venue_name, p.party_name, p.category, p.its_no, p.leader_name,
     ...
     JOIN parties p ON p.id = s.party_id
     WHERE s.miqaat_id = :miqaat_id${zoneSql}
     ORDER BY z.zone_name, m.mohallah_name, v.venue_name, p.party_name`, params);
    return (0, response_1.ok)(res, rows, "OK");
});
exports.reportsRoutes.get("/attendance", async (req, res) => {
    const user = req.user;
    const miqaatId = req.query.miqaat_id ? Number(req.query.miqaat_id) : null;
    if (!Number.isFinite(miqaatId))
        return (0, response_1.fail)(res, "miqaat_id is required", 400);
    const zoneId = getZoneScope(user, req.query.zone_id);
    const params = { miqaat_id: miqaatId, coordinator_id: user.role === "zonal_head" ? 0 : 0 };
    let zoneSql = "";
    if (Number.isFinite(zoneId)) {
        zoneSql = " AND m.zone_id = :zone_id";
        params.zone_id = zoneId;
    }
    const [rows] = await pool_1.pool.query(`SELECT s.id AS schedule_id, z.zone_name, m.mohallah_name, v.venue_name, p.party_name, p.category,
...
exports.reportsRoutes.get("/performance", async (req, res) => {
    const user = req.user;
    const partyId = req.query.party_id ? Number(req.query.party_id) : null;
    const zoneId = getZoneScope(user, req.query.zone_id);
    const params = {};
    ...
    if (Number.isFinite(partyId)) {
        ...
        const [trend] = await pool_1.pool.query(`SELECT q.id AS miqaat_id, q.miqaat_name, q.english_date,
       ...
       ORDER BY q.english_date DESC`, { party_id: partyId });
        const [mic] = await pool_1.pool.query(`SELECT q.id AS miqaat_id, q.miqaat_name, q.english_date,
       ...
       GROUP BY q.id, q.miqaat_name, q.english_date
       ORDER BY q.english_date DESC`, { party_id: partyId });
        return (0, response_1.ok)(res, { performance_trend: trend, mic_trend: mic }, "OK");
    }
    const [rows] = await pool_1.pool.query(`SELECT p.id AS party_id, p.party_name, p.category, z.zone_name,
            AVG(pr.overall_score) AS avg_overall,
...
exports.reportsRoutes.get("/quarterly", async (req, res) => {
    const user = req.user;
    const year = Number(req.query.year);
    const quarter = Number(req.query.quarter);
    if (!Number.isFinite(year) || !Number.isFinite(quarter) || quarter < 1 || quarter > 4) {
        return (0, response_1.fail)(res, "year and quarter (1-4) are required", 400);
    }
    ...
    return (0, response_1.ok)(res, { best, worst }, "OK");
});
exports.reportsRoutes.get("/manually-edited", async (req, res) => {
    const user = req.user;
    const zoneId = getZoneScope(user, req.query.zone_id);
    const miqaatId = req.query.miqaat_id ? Number(req.query.miqaat_id) : null;
    const params = {};
    ...
    const [rows] = await pool_1.pool.query(`SELECT e.id AS edit_id, e.schedule_id, e.edited_by_role, e.edited_by_id, e.edited_at,
     ...
     ${whereSql}
     ORDER BY e.edited_at DESC`, params);
    return (0, response_1.ok)(res, rows, "OK");
});
